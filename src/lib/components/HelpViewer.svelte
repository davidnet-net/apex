<script lang="ts">
	...
	const { document, githubpath, docname } = $props<{ document: string; githubpath: string; docname: string }>();
...
<FlexWrapper height="100%" width="100%">
	...
	<FlexWrapper height="100%" width="100%" justifycontent="flex-start" gap="var(--token-space-1);">
		<h1 style="font-size: 3rem">{docname}</h1>
		...
		<FlexWrapper>
			<FlexWrapper direction="row">
				...
				<LinkButton iconbefore="help " href="/help">Help</LinkButton>
			</FlexWrapper>
...
<style>
	...
	:global(.markdown th, .markdown td) {
		border: 2px solid var(--token-color-text-default-tertiary);
		background: none;
		color: inherit;
		box-shadow: none;
		text-shadow: none;
